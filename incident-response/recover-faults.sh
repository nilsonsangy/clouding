...
usage() {
  echo "Usage: sh incident-response/recover-faults.sh <docker|k8s> <latency|errors|crash|all> [service|all] [namespace]"
  echo "Examples:"
  echo "  sh incident-response/recover-faults.sh docker all"
  echo "  sh incident-response/recover-faults.sh docker latency orders"
  echo "  sh incident-response/recover-faults.sh k8s errors catalog clouding-lab"
}
