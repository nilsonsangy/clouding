...
usage() {
	echo "Usage: sh incident-response/inject-faults.sh <docker|k8s> <latency|errors|crash> [service] [namespace]"
	echo "Examples:"
	echo "  sh incident-response/inject-faults.sh docker latency orders"
	echo "  sh incident-response/inject-faults.sh docker errors catalog"
	echo "  sh incident-response/inject-faults.sh k8s crash orders clouding-lab"
}
