...
1. Build and push the application images to Amazon ECR.
   ...
   docker build -t clouding-users:$env:IMAGE_TAG services/users
   docker build -t clouding-orders:$env:IMAGE_TAG services/orders
   docker build -t clouding-catalog:$env:IMAGE_TAG services/catalog
...
2. Define and deploy the ECS Fargate Infrastructure.
   Instead of running ECS commands manually, we provide a pre-built CloudFormation template (`deploy/aws/ecs-fargate.yml`) that creates the ECS cluster, Fargate capacities, networking, and the 4 services mapped to your ECR images.

   ```powershell
   aws cloudformation deploy `
     --template-file deploy/aws/ecs-fargate.yml `
