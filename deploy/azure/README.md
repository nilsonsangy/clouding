...
docker build -t clouding-users:$IMAGE_TAG services/users
docker build -t clouding-orders:$IMAGE_TAG services/orders
docker build -t clouding-catalog:$IMAGE_TAG services/catalog
...
docker compose -f deploy/azure/docker-compose.yml pull
docker compose -f deploy/azure/docker-compose.yml up -d
```
...
```bash
kubectl apply -f deploy/azure/k8s/namespace.yaml
